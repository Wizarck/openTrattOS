...
- Span enricher interceptor adds `nexandro.tag` JSONB attribute to every span, drives slice #20 widget #7 (cost-by-tag drill-down).
...
### ADR-VISION-TAG-ATTRIBUTE — `nexandro.tag` attribute is the cost drill-down primary key

Every span emitted via `OtelService.startSpan(name, options)` MUST include an `nexandro.tag` attribute. The interceptor `SpanEnricherInterceptor` enforces this — if a span is sealed without the tag, the interceptor logs a warning at `warn` level and assigns `nexandro.tag = 'untagged'` so the span still flows but is visibly mis-tagged in the dashboard.
...
`VisionLlmFactory` reads `NEXANDRO_VISION_LLM_PROVIDER` (default: `gpt-oss-vision-rag-proxy`) at `onModuleInit()` and resolves the appropriate adapter:
...
| Env | Default | Purpose |
|---|---|---|
| `NEXANDRO_OTEL_EXPORTER_ENDPOINT` | `http://localhost:4318/v1/traces` | OTLP/HTTP endpoint |
| `NEXANDRO_OTEL_EXPORTER_HEADERS` | (empty) | Comma-separated `key=value` pairs for tenant auth (e.g. `x-langfuse-key=...,x-otel-tenant=org-XYZ`) |
| `NEXANDRO_OTEL_SERVICE_NAME` | `nexandro-api` | Span `service.name` resource attribute |
| `NEXANDRO_OTEL_DISABLED` | `false` | Set `true` to disable exporter entirely (spans still emit in-process but discarded) |

`NEXANDRO_OTEL_DISABLED=true` is the rollback path — disable telemetry without redeploying.
...
- **[Risk]** `nexandro.tag = 'untagged'` warnings could go unnoticed at dashboard-review time. **Mitigation**: slice #20 widget #7 surfaces `untagged` as a paprika-coloured row at the top of the cost-by-tag table — visually impossible to miss.
...
1. **Stage 1 — Schema + module wiring** (this PR):
   ...
   - Environment defaults to OTel disabled (`NEXANDRO_OTEL_DISABLED=true` in `.env.development`) until staging is wired.
2. **Stage 2 — Staging telemetry validation**:
   - Set `NEXANDRO_OTEL_EXPORTER_ENDPOINT=https://otel-collector.staging.example/v1/traces` on staging.
...
4. **Rollback strategy**:
   - `NEXANDRO_OTEL_DISABLED=true` disables exporter without redeploy.
...
- **`nexandro.tag` validation strictness**: should the interceptor REJECT spans without a tag (raise error), or just warn-and-default-to-untagged? **Proposed answer**: warn-only in this slice; slice #20 dashboard surfaces `untagged` row prominently. Strict rejection deferred to a future ai-playbook-style hook.
