...
- [ ] §7.2 — Set `ghcr.io/wizarck/opentrattos` package visibility to public via GHCR UI (one-time).
- [ ] §7.3 — On VPS: `mkdir -p /opt/opentrattos`, scp `deploy/docker-compose.prod.yml` and a populated `.env` (with operator-generated `POSTGRES_PASSWORD`).
- [ ] §7.4 — On VPS: `cd /opt/opentrattos && docker compose pull && docker compose up -d`. Verify `docker ps` shows both services healthy.
...
- [ ] §7.7 — User creates Cloudflare DNS CNAME `trattos.palafitofood.com` → `675fa973-4c22-4b1c-9fd4-a52fad422ca4.cfargotunnel.com` (proxied).
- [ ] §7.8 — From desktop: `curl https://trattos.palafitofood.com/health` → 200. `open https://trattos.palafitofood.com` shows SPA.
