...
A new bounded context — separated from `photo-ingestion` because routing/revocation is a *cross-aggregate* concern that may legitimately scale to a separate microservice in TrattOS Enterprise. The BC's only job is to consume `HITL_RETROACTIVE_CORRECTION` and produce the downstream-flagging envelopes.
...
- **`packages/mcp-server-opentrattos/src/capabilities/write/inventory.ts`** — add `inventory.retroactive-correct-photo-ingestion`:
...
- **`packages/mcp-server-opentrattos/src/capabilities/write/inventory.spec.ts`** — extend with shape + schema tests for the new capability.
- **`packages/mcp-server-opentrattos/src/capabilities/write/index.spec.ts`** — count 52 → 53 (or whatever the current barrel total is — see tasks.md §0 pre-flight).
- **`packages/mcp-server-opentrattos/test/smoke.spec.ts`** — count 59 → 60.
