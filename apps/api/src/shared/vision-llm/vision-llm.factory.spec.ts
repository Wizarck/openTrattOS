...
describe('VisionLlmFactory (env-driven adapter selection)', () => {
  const originalEnv = process.env.OPENTRATTOS_VISION_LLM_PROVIDER;

  afterEach(() => {
    if (originalEnv === undefined) {
      delete process.env.OPENTRATTOS_VISION_LLM_PROVIDER;
    } else {
      process.env.OPENTRATTOS_VISION_LLM_PROVIDER = originalEnv;
    }
  });
  ...
  it('defaults to GptOssVisionRagProxyProvider when env is empty string', () => {
    process.env.OPENTRATTOS_VISION_LLM_PROVIDER = '   ';
  ...
  it('selects ClaudeVisionProvider for env=claude-vision', () => {
    process.env.OPENTRATTOS_VISION_LLM_PROVIDER = 'claude-vision';
    const provider = build().getProvider();
  ...
  it('selects GptFourVProvider for env=gpt-four-v', () => {
    process.env.OPENTRATTOS_VISION_LLM_PROVIDER = 'gpt-four-v';
    const provider = build().getProvider();
  ...
  it('throws UnknownVisionLlmProviderError at construction (NOT at first call) for unknown env value', () => {
    process.env.OPENTRATTOS_VISION_LLM_PROVIDER = 'acme-vision';
