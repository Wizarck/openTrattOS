...
describe('createAuditArchiveStorage', () => {
  ...
  beforeEach(() => {
    prevBackend = process.env.NEXANDRO_AUDIT_ARCHIVE_BACKEND;
    prevBucket = process.env.NEXANDRO_AUDIT_ARCHIVE_S3_BUCKET;
  });

  afterEach(() => {
    if (prevBackend === undefined) {
      delete process.env.NEXANDRO_AUDIT_ARCHIVE_BACKEND;
    } else {
      process.env.NEXANDRO_AUDIT_ARCHIVE_BACKEND = prevBackend;
    }
    if (prevBucket === undefined) {
      delete process.env.NEXANDRO_AUDIT_ARCHIVE_S3_BUCKET;
    } else {
      process.env.NEXANDRO_AUDIT_ARCHIVE_S3_BUCKET = prevBucket;
    }
  });
  ...
  it('returns FilesystemArchiveStorage when env is "filesystem"', () => {
    process.env.NEXANDRO_AUDIT_ARCHIVE_BACKEND = 'filesystem';
    expect(createAuditArchiveStorage()).toBeInstanceOf(
      FilesystemArchiveStorage,
    );
  });
  ...
  it('returns FilesystemArchiveStorage for unknown backend values', () => {
    process.env.NEXANDRO_AUDIT_ARCHIVE_BACKEND = 'azure-direct';
    expect(createAuditArchiveStorage()).toBeInstanceOf(
      FilesystemArchiveStorage,
    );
  });
  ...
  it('returns S3CompatibleArchiveStorage when env is "s3"', () => {
    process.env.NEXANDRO_AUDIT_ARCHIVE_BACKEND = 's3';
    process.env.NEXANDRO_AUDIT_ARCHIVE_S3_BUCKET = 'b';
    expect(createAuditArchiveStorage()).toBeInstanceOf(
